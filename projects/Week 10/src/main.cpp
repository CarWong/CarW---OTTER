...
void CreateScene() {
	...
	if (loadScene) {
		...
		Shader::Sptr redShader = ResourceManager::CreateAsset<Shader>(std::unordered_map < ShaderPartType, std::string>
		{
			{ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl"},
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/removered.glsl" }
		});
		...
		redTex->SetMinFilter(MinFilter::Nearest);
		redTex->SetMagFilter(MagFilter::Nearest);
		...
		Material::Sptr redMaterial = ResourceManager::CreateAsset<Material>(redShader);
		{
			redMaterial->Name = "Red";
			redMaterial->Set("u_Material.Diffuse", redTex);
			redMaterial->Set("u_Material.Shininess", 0.1f);
			redMaterial->Set("u_Material.Threshold", 0.1f);
		}
